...
var codepen_link = document.getElementById('codepen_link');
// facebook_link.href='javascript:void(0);';

let array = ["name","website","linkedin","medium","twitter","github","codepen"];
...
chrome.storage.sync.get(array,function(links){
    if(!chrome.runtime.error){
        ...
        if(links.codepen)
            codepen_link.href=links.codepen;
        else
            codepen_link.addEventListener('click',function(event){
                event.preventDefault();
            });
